...
vkus?: "ягодный"| "ванильный" | "шоколадный" | "карамельный"){
    let itogSum = 0;
...
function arrReturn(arr : string | number | number[] | null){
    let res = [];
...
class Student{
    ...
    constructor(surname: string, name:string, secondname?:string){
...
class Group{
    ...
    constructor(title: string, nameOfCourse: string, listStudent: Student[]){
        this.title = title;
...
class Teacher{
    ...
    constructor(surname: string, name: string, secondname?: string, listGroup?: Group[], degree?: string){
        this.surname = surname;
        this.name = name;
...
function getName(o: Student | Group | Teacher){
    if(o instanceof Student){
        if(o.secondname==null) return o.surname + " " + o.name;
        else return o.surname + " " + o.name + " " + o.secondname;
    }
...
function studentCount(o: Teacher | Group){
    let count = 0;
    if(o instanceof Teacher){
        if(o.listGroup){
...
function createElementWithArgs(tag: string, args: [string, string][], inner?: string) : HTMLElement {
